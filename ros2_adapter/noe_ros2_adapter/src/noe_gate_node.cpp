...
//   4. Call noe::evaluate(chain_, context_json, mode_) through the C FFI
//   boundary.
...
//   allocation before returning a std::string. No manual memory management
//   here.
...
CallbackReturn
NoeGateNode::on_configure(const rclcpp_lifecycle::State & /*state*/) {
  ...
  if (mode_ != "strict" && mode_ != "partial") {
    RCLCPP_ERROR(get_logger(),
                 "Invalid mode '%s' — must be 'strict' or 'partial'",
                 mode_.c_str());
...
CallbackReturn NoeGateNode::on_activate(const rclcpp_lifecycle::State &state) {
  ...
  // Subscriptions
  sub_zone_clear_ = create_subscription<std_msgs::msg::Bool>(
      "/noe/zone_clear", 10,
      [this](const std_msgs::msg::Bool::SharedPtr msg) { on_zone_clear(msg); });
  ...
  RCLCPP_INFO(get_logger(),
              "NoeGateNode active. Listening on /noe/proposed_action.");
  return CallbackReturn::SUCCESS;
}
...
CallbackReturn
NoeGateNode::on_deactivate(const rclcpp_lifecycle::State &state) {
...
CallbackReturn
NoeGateNode::on_cleanup(const rclcpp_lifecycle::State & /*state*/) {
  {
...
CallbackReturn
NoeGateNode::on_shutdown(const rclcpp_lifecycle::State & /*state*/) {
  active_.store(false);
  sub_zone_clear_.reset();
  sub_proposed_action_.reset();
  pub_permitted_.reset();
...
void NoeGateNode::on_zone_clear(const std_msgs::msg::Bool::SharedPtr msg) {
  if (!active_.load())
    return;
  std::lock_guard<std::mutex> lk(sensor_mutex_);
  sensor_.zone_clear = msg->data;
  sensor_.received_at_ms = now_ms();
  RCLCPP_DEBUG(get_logger(), "zone_clear updated: %s",
               msg->data ? "true" : "false");
}

void NoeGateNode::on_proposed_action(
...
void NoeGateNode::evaluate_and_publish(const std::string &proposed_action,
                                       int64_t current_ms) {
  ...
  if (stale) {
    ...
    int64_t age_ms =
        (snap.received_at_ms == 0) ? -1 : (current_ms - snap.received_at_ms);
    std::string stale_msg =
        "sensor data stale or never received (age=" + std::to_string(age_ms) +
        "ms, max=" + std::to_string(max_sensor_age_ms_) + "ms)";
    nlohmann::json err;
    err["domain"] = "error";
    ...
    err["meta"]["mode"] = mode_;
    err["meta"]["context_hashes"] = {
        {"root", ""}, {"domain", ""}, {"local", ""}, {"total", ""}};
    ...
    params.zone_clear = snap.zone_clear;
    ...
    RCLCPP_INFO(get_logger(), "Noe decision: %s (domain=%s, proposed='%s')",
                decision.decision_str.c_str(), decision.domain.c_str(),
  ...
  std::string cert_line =
      build_cert_record(proposed_action, decision.result_json,
                        decision.decision_str, snap.zone_clear, current_ms);
  append_cert_record(cert_line);
}

// ─── Certificate logging ─────────────────────────────────────────────────────

std::string NoeGateNode::build_cert_record(const std::string &proposed_action,
  ...
  record["context_summary"]["zone_clear"] = zone_clear_snapshot;
